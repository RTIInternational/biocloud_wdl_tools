...
task gsem_preprocessing {
  ...
  command <<<
    ...
    python3 /app/gsem_preprocessing.py \
...
task gsem_sumstats {
  ...
  output {
    ...
    File sumstats_tsv = output_prefix + ".tsv"
  }
...
task gsem_commonfactorgwas {
  ...
  output {
    ...
    File? commonfactorgwas_tsv = output_prefix + ".tsv"
    Array[File] commonfactorgwas_tsvs = glob(output_prefix + "*.tsv")
  }
...
task gsem_usergwas {
  ...
  output {
    ...
    File? usergwas_tsv = output_prefix + ".tsv"
    Array[File] usergwas_tsvs = glob(output_prefix + "*.tsv")
  }
...
task gsem_merge_rds {
  ...
  parameter_meta {
    ...
    rds_file_list: "Optional text file containing a list of RDS file paths to merge."
    output_prefix: "Output prefix used for merged .rds and optional .tsv files."
    docker_image: "Docker image name/tag used when image_source is docker."
    ecr_image: "ECR image name/tag used when image_source is ecr."
    ecr_repo: "Optional ECR repository URI prefix used with ecr_image."
    image_source: "Container source selector: docker or ecr."
  ...
  command <<<
    ...
    Rscript /opt/gsem_merge_rds.R \
      ~{if defined(rds_files) then "--rds_files \"~{sep(",", select_first([rds_files]))}\"" else ""} \
      ~{if defined(rds_file_list) then "--rds_file_list \"" + select_first([rds_file_list]) + "\"" else ""} \
      --output_prefix "~{output_prefix}"
  >>>

  output {
    File merged_rds = output_prefix + ".rds"
    File? merged_tsv = output_prefix + ".tsv"
  }
